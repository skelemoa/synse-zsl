...
nouns = nouns_vocab[np.argmax(np.load('../resources/nouns_ohe.npy'), -1)][:num_class]
verbs = verbs_vocab[np.argmax(np.load('../resources/verbs_ohe.npy'), -1)][:num_class]
...
verb_emb = torch.from_numpy(np.load('../resources/ntu' + str(num_class) + '_' + le + '_verb.npy')).view([num_class, verb_emb_input_size])
verb_emb = verb_emb/torch.norm(verb_emb, dim = 1).view([num_class, 1]).repeat([1, verb_emb_input_size])
noun_emb = torch.from_numpy(np.load('../resources/ntu' + str(num_class) + '_' + le + '_noun.npy')).view([num_class, noun_emb_input_size])
