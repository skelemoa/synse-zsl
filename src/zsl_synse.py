...
from alignment_module import AE, PosMmen, JPosMmen, contractive_loss, MMDLoss, multi_class_hinge_loss, triplet_loss 
...
NounPosMmen = PosMmen(vis_emb_input_size, noun_emb_input_size, vis_hidden_size, text_hidden_size, output_size).to(device)
...
VerbPosMmen = PosMmen(vis_emb_input_size, verb_emb_input_size, vis_hidden_size, text_hidden_size, output_size).to(device)
...
def train(train_loader, epoch, margin):
    ...
    for i, (inputs, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            _, _, verb_embeddings = VerbPosMmen.TextArch(seen_verb_emb.to(device).float())
            _, _, noun_embeddings = NounPosMmen.TextArch(seen_noun_emb.to(device).float())
        ...
        vis_verb_hidden, vis_verb_out, vis_verb_transform, verb_hidden, verb_out, verb_transform = VerbPosMmen(emb, verb_emb_target.to(device).float())
        ...
        verb_vis_mmd_loss = MMDLoss(vis_verb_hidden, verb_hidden)
        verb_verb_loss = triplet_loss(vis_verb_transform, torch.tensor(verb_tar), device, margin).to(device)


        vis_noun_hidden, vis_noun_out, vis_noun_transform, noun_hidden, noun_out, noun_transform = NounPosMmen(emb, noun_emb_target.to(device).float())
        ...
        noun_vis_mmd_loss = MMDLoss(vis_noun_hidden, noun_hidden)
...
def zsl_validate(val_loader, epoch, margin):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, target) in enumerate(val_loader):
            ...
            emb = emb/torch.norm(emb, dim = 1).view([emb.size(0), 1]).repeat([1, emb.shape[1]])
            _, _, verb_embeddings = VerbPosMmen.TextArch(unseen_verb_emb.to(device).float())
            _, _, noun_embeddings = NounPosMmen.TextArch(unseen_noun_emb.to(device).float())
            noun_embeddings = noun_embeddings/torch.norm(noun_embeddings, dim = 1).view([noun_embeddings.size(0), 1]).repeat([1, noun_embeddings.shape[1]])
            verb_embeddings = verb_embeddings/torch.norm(verb_embeddings, dim = 1).view([verb_embeddings.size(0), 1]).repeat([1, verb_embeddings.shape[1]])
            joint_text_embedding = torch.cat([verb_embeddings, noun_embeddings], axis=1)
            fin_text_embedding = JointMmen.TextArch(joint_text_embedding)
...
def gzsl_validate(val_loader, epoch, margin):

    with torch.no_grad():
        losses = AverageMeter()
        acces = AverageMeter()
        vaccess = AverageMeter()
        naccess = AverageMeter()
        ce_loss_vals = []
        ce_acc_vals = []
        NounPosMmen.eval()
        VerbPosMmen.eval()
        ...
        for i, (inputs, target) in enumerate(val_loader):
            ...
            emb = emb/torch.norm(emb, dim = 1).view([emb.size(0), 1]).repeat([1, emb.shape[1]])
            _, _, verb_embeddings = VerbPosMmen.TextArch(verb_emb.to(device).float())
            _, _, noun_embeddings = NounPosMmen.TextArch(noun_emb.to(device).float())
            noun_embeddings = noun_embeddings/torch.norm(noun_embeddings, dim = 1).view([noun_embeddings.size(0), 1]).repeat([1, noun_embeddings.shape[1]])
            verb_embeddings = verb_embeddings/torch.norm(verb_embeddings, dim = 1).view([verb_embeddings.size(0), 1]).repeat([1, verb_embeddings.shape[1]])
            joint_text_embedding = torch.cat([verb_embeddings, noun_embeddings], axis=1)
            fin_text_embedding = JointMmen.TextArch(joint_text_embedding)
            ...
            # ce acc
            # verb_acc, verb_pred = accuracy(verb_embeddings, vis_verb_transform, target.to(device), gzsl_inds)
            # noun_acc, noun_pred = accuracy(noun_embeddings, vis_noun_transform, target.to(device), gzsl_inds)
            ce_acc, pred = accuracy(fin_text_embedding, fin_vis, target.to(device), gzsl_inds)
            
            losses.update(loss.item(), inputs.size(0))
            acces.update(ce_acc, inputs.size(0))
            # vaccess.update(verb_acc, inputs.size(0))
            # naccess.update(noun_acc, inputs.size(0))
            preds.append(pred)
            # vpreds.append(verb_pred)
            # npreds.append(noun_pred)
            tars.append(target)
            ce_loss_vals.append(loss.cpu().detach().numpy())
            ce_acc_vals.append(ce_acc)
...
for epoch in range(0, max_epochs):
        ...
        zsl_loss, zsl_acc, _ , _ = zsl_validate(zsl_loader, epoch, margin)
        # gzsl_loss, gzsl_acc, p, t = gzsl_validate(val_loader, epoch, margin)
        # p = np.array([j.cpu().numpy() for i in p for j in i])
        # t = np.array([j.cpu().numpy() for i in t for j in i])
        # cmat = confusion_matrix(t, p)
        # print(cmat.shape)
        # unseen_acc = 0
        # seen_acc = 0
        ####################################################
        # for ind in unseen_inds:
        #     i = np.argwhere(gzsl_inds == ind)[0][0]
        #     unseen_acc += cmat[i, i]/np.sum(cmat[i])
        #     print(labels[ind], ' : ', cmat[i, i]/np.sum(cmat[i]))
        #     print(labels[unseen_inds[np.argsort(cmat[i])[::-1]]])
        ...
        # print('seen acc: {:.4f}\t'
        #       'unseen acc: {:.4f}\t'
        #       'h mean: {:.4f}\t'.format(seen_acc, unseen_acc, h_mean))
        ...
        if np.greater(current, best):
            print('Epoch %d: %sd from %.4f to %.4f, '
                'saving model to %s'
                % (epoch , 'zsl improved', best, current, npm_zcheckpoint))
            best = current
            best_epoch = epoch
